	...
	ItemType &target, bool &success)
{
	if (treeRoot ==NULL){
		...
		if(target == treeRoot->getItem()){
			...
			//if the target is less than the data item in the root of the tree

			// then call the function itself and pass the root of left subtree and target
			//as well as the variable success to the function. the function then will
			//remove the data item from the left subtree and return the new root of the 
			// left subtree. the function will set the leftchildptr of the treeroot to
			//the new root of the left subtree.
		}
	}
}
template<class ItemType>
